...
* `--primers-linked TRUE/FALSE` - if set to TRUE, instructions cutadapt to treat the primers as linked. Default: FALSE

* `--anchor-primers TRUE/FALSE` - indicates if primers should be anchored (TRUE) or not (FALSE).
  Default for un-linked primers: FALSE
  Default for linked primers: TRUE
...
* `--left-maxEE` - specifies the maximum expected error (maxEE) allowed for each forward read, reads with higher than maxEE will be discarded. Default: Inf (no expected error filtering)
   > *Note: See dada2 [filterAndTrim documentation](https://rdrr.io/bioc/dada2/man/filterAndTrim.html) for more info.*

* `--right-maxEE` - specifies the maximum expected error (maxEE) allowed for each forward read, reads with higher than maxEE will be discarded. Default: Inf (no expected error filtering)
   > *Note: See dada2 [filterAndTrim documentation](https://rdrr.io/bioc/dada2/man/filterAndTrim.html) for more info.*
