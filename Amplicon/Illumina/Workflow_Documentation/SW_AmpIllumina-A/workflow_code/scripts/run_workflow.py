...
# 3. Create config.yaml and unique_sample_IDs.txt
...
def sample_IDs_from_local(runsheet_df, output_file='unique_sample_IDs.txt'):
...
def handle_url_downloads(runsheet_df, output_file='unique_sample_IDs.txt'):
    # Check if the DataFrame is paired-end
    paired_end = runsheet_df['paired_end'].eq(True).all()
    # Write 'Sample Name' into unique_sample_IDs.txt
    with open(output_file, 'w') as file:
...
def main():
    ...
    # Create unique_sample_IDs.txt based on filenames or 'Sample Name' if URLs
    ...
    if args.OSD or args.runsheetPath:
        if runsheet_file:
            ...
            if runsheet_df is not None:
                ...
                # Create the 'unique_sample_IDs.txt' file and download read files if necessary
                if uses_urls:
                    handle_url_downloads(runsheet_df, output_file='unique_sample_IDs.txt')
                else:
                    sample_IDs_from_local(runsheet_df, output_file='unique_sample_IDs.txt')
