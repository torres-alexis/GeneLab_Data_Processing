...
- `--site-reports 1` – generate site reports (0 = no, 1 = yes)
...
- \*.mzML (input mass spectrometry raw data in mzML format, specified with `--specdir` in filelist_ionquant.txt)
...
- **FragPipeAnalystR_parameters_GLProteomics.txt** (run parameters)
- **nonimputed_matrix_GLProteomics.csv** (from combined_protein/peptide: contaminants removed; selected `--lfq_type` quantification columns reappended at the end of the table as either log2 intensity (Intensity/MaxLFQ) or raw counts (Spectral Count). NAs where feature not detected.)
- **imputed_matrix_GLProteomics.csv** (same structure as nonimputed_matrix_GLProteomics.csv; NAs filled by Perseus-type imputation: missing values replaced with random numbers sampled from a normal distribution with mean shifted 1.8 standard deviations below and a width (SD) of 0.3, per sample.)
- **QC_plots_GLProteomics.zip** (QC plots folder)
...
- **comparison_plots_GLProteomics.zip** (comparison plots folder)
...
- **pathway_analysis_plots_GLProteomics.zip** (pathway analysis plots folder)
...
- **DE_plots_GLProteomics.zip** (DE plots folder)
...
- **DE_results.csv** (differential expression results table; columns in order:
    ...
    - Protein level:
      - Protein (FASTA header for the protein group; inferred from combined peptide evidence; razor protein when evidence maps to multiple proteins)
      - Protein.ID (protein identifier / primary accession number)
      - Entry.Name (protein entry name)
      - Gene (gene name)
      - Protein.Length (number of amino acid residues in the protein)
      - Coverage (percent of total protein length covered by identified peptides)
      - Organism (species)
      - Protein.Existence (UniProt protein existence evidence type)
      - Description (protein name)
      - Protein.Probability (ProteinProphet confidence score from combined evidence; higher values indicate greater confidence)
      - Top.Peptide.Probability (highest PeptideProphet score among all peptides mapped to the protein)
      - Combined.Total.Peptides (stripped peptides mappable to the protein; counts depend on protein inference when peptides map to multiple proteins)
      ...
      - \*.Spectral.Count (per-sample PSMs for razor peptides)
      - \*.Unique.Spectral.Count (per-sample PSMs for unique peptides)
      - \*.Total.Spectral.Count (per-sample PSMs for total peptides)
      - \*.Intensity (per-sample normalized intensity from razor peptides, top-N algorithm; normalized by default)
      - \*.Unique.Intensity (per-sample normalized intensity from unique peptides, top-N algorithm; normalized by default)
      - \*.Total.Intensity (per-sample normalized intensity from total peptides, top-N algorithm; normalized by default)
      - \*.MaxLFQ.Intensity (per-sample MaxLFQ-normalized intensity from razor peptides; normalized by default)
      - \*.MaxLFQ.Unique.Intensity (per-sample MaxLFQ-normalized intensity from unique peptides; normalized by default)
      - \*.MaxLFQ.Total.Intensity (per-sample MaxLFQ-normalized intensity from total peptides; normalized by default)
      - Indistinguishable.Proteins (proteins that cannot be distinguished from the identification in the `Protein` column given all sequences and evidence in the experiment)
      - name (FragPipeAnalystR: plot/table label from `Gene`, or `Protein ID` if `Gene` is empty)
      - ID (FragPipeAnalystR: copy of `Protein ID`)
    - Peptide level:
      - Peptide.Sequence (stripped peptide amino acid sequence; modifications not included)
      - Prev.AA (amino acid residue immediately preceding the peptide)
      - Next.AA (amino acid residue immediately following the peptide)
      ...
      - Protein (FASTA protein sequence header for the identified peptide; razor protein when the peptide maps to multiple proteins; other mappings in `Mapped Proteins`)
      - Protein.ID (primary accession number)
      - Entry.Name (protein entry name)
      - Gene (gene name from the razor protein when the peptide maps to multiple proteins)
      - Description (name / description of the parent protein)
      - Mapped.Genes (additional genes the peptide may originate from, including any from I/L substitutions)
      - Mapped.Proteins (additional proteins the peptide maps to, including any from I/L substitutions)
      - \*.Spectral.Count (PSM count in the sample supporting the peptide identification)
      - \*.Intensity (per-sample normalized peptide intensities)
      - \*.MaxLFQ.Intensity (per-sample MaxLFQ-normalized peptide intensities; present when MaxLFQ is enabled in FragPipe)
      - \*.Match.Type (per-sample; direct = observed in run, transferred = matched between runs)
      - Index (unique peptide row key: `Protein ID` + stripped `Peptide Sequence`, or + `Modified Sequence` when that column is present in the input)
      - name (FragPipeAnalystR: plot/table label from `Protein ID`)
