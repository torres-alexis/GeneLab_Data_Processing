# GeneLab bioinformatics processing pipeline for Mass Spectrometry-based Proteomics Data (TMT-16 Phosphoproteomics Workflow)

> **This page holds an overview and instructions for how GeneLab processes mass spectrometry-based phosphoproteomics data using the TMT-16 (Tandem Mass Tag 16-plex) workflow. Exact processing commands, GL-DPPD-[TMT16-phospho] version used, and processed data output files for specific datasets are provided in the [Open Science Data Repository (OSDR)](https://osdr.nasa.gov/bio/repo/).**  

---

**Date:** January X, 2026  
**Revision:** A  
**Document Number:** GL-DPPD-[TMT16-phospho]-A  
...
- [**General processing overview with example commands**](#general-processing-overview-with-example-commands)
  ...
  - [**3. FragPipe Processing Pipeline**](#3-fragpipe-processing-pipeline)
    ...
    - [3f. PTMProphet PTM Localization](#3f-ptmprophet-ptm-localization)
...
|PTMProphet|7.3.0|[https://github.com/Nesvilab/PTMProphet](https://github.com/Nesvilab/PTMProphet)|
...
<img src="../Workflow_Documentation/NF_Proteomics/images/draft_pipeline.png" align="center" alt="Proteomics TMT-16 phosphoproteomics processing workflow"/>
...
- `TMT16-phospho.workflow` (FragPipe workflow configuration file for TMT-16 phosphoproteomics workflow)
...
- `--only-psms` – do not remove redundant peptides, keep PSMs, exclude peptide level 
probabilities
...
- `*.pin` – input Percolator input files (tab-delimited pin-tab format)
...
### 3f. PTMProphet PTM Localization
...
- `KEEPOLD` – keep old PTMProphet results in the pepXML file (default: off, old results are overwritten)
- `STATIC` – use static fragment tolerance
- `FRAGPPMTOL=10` – +/- MS2 mz tolerance on fragment ions in PPM (default: estimate from the data)
- `EM=1` – EM models: 1 = Intensity EM Model Applied (default: 1)
- `NIONS=b` – use specified N-term ions (default: b for CID)
- `M:15.9949,STY:79.96633` – modification specification format: `<amino acids>:<mass_shift>`. M:15.9949 = oxidation on methionine, STY:79.96633 = phosphorylation on serine/threonine/tyrosine (default: STY:79.9663,MW:15.9949)
- `MINPROB=0.5` – minimum probability to evaluate peptides (default: 0)
- `MAXTHREADS=1` – number of threads for processing (0 uses all available processors, default: 1)
- `interact-*.pep.xml` – input pepXML files
- `interact-*.mod.pep.xml` – output pepXML files with localization information

**Input Data:**

- `interact-*.pep.xml` (pepXML files with Percolator validation information, output from [Step 3e.1](#3e1-add-percolator-validation-information-to-pepxml))
...
- `interact-*.mod.pep.xml` (pepXML files with PTM localization information, output from [Step 3f](#3f-ptmprophet-ptm-localization))
...
- `interact-*.mod.pep.xml` (peptide-spectrum matches with PTM localization information, output from [Step 3f](#3f-ptmprophet-ptm-localization))
