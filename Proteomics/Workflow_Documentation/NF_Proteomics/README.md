...
The current GeneLab Proteomics processing pipeline (PPP) ([GL-DPPD-XXXX-A](https://github.com/nasa/GeneLab_Data_Processing/tree/master/Proteomics/Pipeline_GL-DPPD-XXXX_Versions/GL-DPPD-XXXX-A.md)) is implemented as a [Nextflow](https://nextflow.io/) DSL2 workflow that utilizes [Singularity](https://docs.sylabs.io/guides/3.10/user-guide/introduction.html) to run all tools in containers. This workflow (NF_P) is run using the command line interface (CLI) of any unix-based system. While knowledge of creating workflows in Nextflow is not required to run the workflow as is, [the Nextflow documentation](https://nextflow.io/docs/latest/index.html) is a useful resource for users who want to modify and/or extend this workflow.   
...
<details open>
<summary>NF_P workflow for GL-DPPD-XXXX-A</summary>
<p align="center">
...
All files required for utilizing the NF_P GeneLab workflow for processing Proteomics data are in the [workflow_code](workflow_code) directory. To get a 
copy of latest NF_P version on to your system, the code can be downloaded as a zip file from the release page then unzipped after downloading by running the following commands: 

```bash
wget https://github.com/nasa/GeneLab_Proteomics_Workflow/releases/download/NF_P_1.0.0/NF_P_1.0.0.zip

unzip NF_P_1.0.0.zip
```
...
While in the location containing the `NF_P_1.0.0` directory that was downloaded in [step 2](#2-download-the-workflow-files), you are now able to run the workflow.

 Below are two examples of how to run the NF_P workflow:
...
```bash
nextflow run NF_P_1.0.0/main.nf \ 
...
* `NF_P_1.0.0/main.nf` - Instructs Nextflow to run the NF_P workflow 
...
```bash
nextflow run NF_P_1.0.0/main.nf --help
```
...
> Note: The outputs from the RNAseq Consensus Pipeline Subworkflow are documented in the [GL-DPPD-XXXX-A](https://github.com/nasa/GeneLab_Data_Processing/tree/master/RNAseq/Pipeline_GL-DPPD-XXXX_Versions/GL-DPPD-XXXX-A.md) processing protocol.
