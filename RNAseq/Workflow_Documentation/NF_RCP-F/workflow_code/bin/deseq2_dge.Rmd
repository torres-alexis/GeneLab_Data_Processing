...
# Technical replicates handling not yet implemented
# if (!is.null(params$technical_replicates_path) && file.exists(params$technical_replicates_path)) {
#   tech_reps <- read.csv(params$technical_replicates_path, header = FALSE, 
#                        col.names = c("SampleWithReps", "CollapsedSampleName"))
# } else {
#   tech_reps <- NULL
# }
...
study <- compare_csv[, -1]  # Exclude sample_id
...
normCounts <- as.data.frame(assay(vst(dds_1)))
...
# CHANGED TO VST FROM NORMCOUNTS + 1. TO DO: VERIFY DOWNSTREAM FUNCTIONALITY 
print(sprintf("DEBUG: %s Printing head of: '%s' below", Sys.time(), 'normCounts'))
print(head(normCounts), quote = TRUE)
...
# Combine with the existing output_table_1
output_table_1 <- cbind(output_table_1, res_df)
```
