...
# Use parallel processing if more than one CPU is specified
if (params$cpus > 1) {
    BiocParallel::register(BiocParallel::MulticoreParam(workers = params$cpus))
}

# ERCC-based size factor estimation with error handling
dds_1 <- tryCatch(
    expr = {
        DESeq2::estimateSizeFactors(dds_1, controlGenes = ercc_rows_gpB, parallel = (params$cpus > 1))
    },
    error = function(e) {
        DESeq2::estimateSizeFactors(dds_1, type = "poscounts", controlGenes = ercc_rows_gpB, parallel = (params$cpus > 1))
    }
)

# Remove ERCCs from the counts table after normalization
dds_1 <- dds_1[-c(ercc_rows),]

# Estimate dispersions and perform Wald test using parallel processing
dds_1 <- DESeq2::estimateDispersions(dds_1, parallel = (params$cpus > 1))
dds_1 <- DESeq2::nbinomWaldTest(dds_1, parallel = (params$cpus > 1))
```
...
cat(paste(session_info_output, 
          "\nBioC_version_associated_with_R_version: ", bioc_version, 
          sep = "\n"),
