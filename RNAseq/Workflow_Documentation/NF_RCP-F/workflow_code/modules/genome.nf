...
process QUANTIFY_BOWTIE2_GENES {
  ...
  output:
    tuple path("FeatureCounts_Unnormalized_Counts_GLbulkRNAseq.csv"), path("FeatureCounts_Unnormalized_Counts_GLbulkRNAseq.csv.summary"), emit: publishables
    path("versions.txt"), emit: version
  script:
    ...
    -t exon \
    -g gene_id \
    -o "FeatureCounts_Unnormalized_Counts_GLbulkRNAseq.csv" \
    ${bamList}
...
process QUANTIFY_NONZERO_GENES {
    ...
    output:
        path("FeatureCounts_NumNonZeroGenes_GLbulkRNAseq.csv"), emit: numNonzeroGenes

    script:
    """
    Quantitate_non-zero_genes_per_sample_FeatureCounts.py ${ counts_file } FeatureCounts_NumNonZeroGenes_GLbulkRNAseq.csv
    """
}